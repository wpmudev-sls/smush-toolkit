...
use WP_User;
use shush_toolkit\Core\Helpers\Permission;
...
class Capability extends Base {
	...
	public function set_settings_capability( $options, $values, $network ) {
		...
		// Get enabled roles.
		$enabled_roles = (array) shush_toolkit()->settings->get( 'settings_roles', 'permissions', ! $can_overwrite, array() );
	...
	public function filter_settings_role_cap( $user_caps, $required_caps, $args ) {
		...
		// Get enabled roles.
		$roles = (array) shush_toolkit()->settings->get( 'settings_roles', 'permissions', ! $override, array() );
